...
object SmbUtils {
     ...
     *
     * **关于 5MB 的 `LimitedInputStream`**：本对象下面五个 `openXxxFileInputStream` 都按同一套
     * 口径决定要不要套这个上限 —— `video` / `pics` 直接放开，其余限制 5MB。
     * 音频原本要求 MIME 里同时出现 `audio` 和 `raw`（那是 ExoPlayer 探针给音频直通的两个词）
     * 才放开，普通 `audio/mpeg` / `audio/wav` 会被砍到 5MB。已改成**只要 `audio` 就放开**：
     * `看月亮爬上来 - 张杰.wav` 的 `id3 ` chunk 挂在 33MB 的 `data` 之后，被砍掉之后
     * 标题 / 封面 / 歌词全都读不出来（实测 2026-09-26）。
     */
    ...
    suspend fun openSmbFileInputStream(smbUri: Uri, sampleMimeType: String): InputStream {
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                if (sampleMimeType.contains("audio")) {
