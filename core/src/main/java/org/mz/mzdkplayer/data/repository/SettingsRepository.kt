...
object SettingsRepository {
    ...
    // --- 手机端（Phone）外观 Keys ---
    /** 主题模式：`PhoneThemeMode` 的枚举名（SYSTEM / LIGHT / DARK） */
    private const val KEY_PHONE_THEME_MODE = "phone_theme_mode"

    /** 是否使用系统动态取色（Material You，仅 Android 12+ 生效） */
    private const val KEY_PHONE_DYNAMIC_COLOR = "phone_dynamic_color"

    /**
     * 手机端：打开目录时是否自动刮削。
     *
     * 与电视端的「刮削源」开关**分工不同**：那六个开关决定「这个来源参不参与刮削」，
     * 这个开关决定「进目录要不要自动开始」。电视端没有「进目录自动刮」这个概念
     * （它是手动点「扫描」），所以这个开关是手机端独有的，默认**关**。
     */
    ...
    /**
     * 手机端主题模式，存 [PhoneThemeMode] 的枚举名。
     *
     * 存枚举名而不是序号：以后往枚举中间插值不会把老用户的设置读错。
     */
    var phoneThemeMode: String
        get() = prefs.getString(KEY_PHONE_THEME_MODE, "SYSTEM") ?: "SYSTEM"
        set(value) = prefs.edit { putString(KEY_PHONE_THEME_MODE, value) }

    /** 手机端是否使用系统动态取色（Material You） */
    var phoneDynamicColor: Boolean
        get() = prefs.getBoolean(KEY_PHONE_DYNAMIC_COLOR, false)
        set(value) = prefs.edit { putBoolean(KEY_PHONE_DYNAMIC_COLOR, value) }

    /** 手机端打开目录时是否自动刮削（默认关，见 [KEY_PHONE_AUTO_SCRAPE] 的注释） */
