...
/**
 * 管理 WebDavConnection 对象的本地存储 (使用 SharedPreferences)
 * @param context Application 或 Activity Context
 */
class WebDavConnectionRepository(private val context: Context) {

    private val prefs: SharedPreferences =
        context.getSharedPreferences("webdav_connections_prefs", Context.MODE_PRIVATE)
    private val gson = Gson()
    private val connectionListType = object : TypeToken<List<WebDavConnection>>() {}.type
     ...
     */
    fun saveConnections(connections: List<WebDavConnection>) {
        prefs.edit {
     ...
     */
    fun getConnections(): List<WebDavConnection> {
        val json = prefs.getString(KEY_CONNECTIONS, null)
        return if (!json.isNullOrEmpty()) {
            try {
                val loadedConnections = gson.fromJson<List<WebDavConnection>>(json, connectionListType)
     ...
     */
    fun addConnection(connection: WebDavConnection) {
        val currentConnections = getConnections().toMutableList()
        // 确保新连接的字段不是 null（使用默认值）
        val safeConnection = WebDavConnection(
            id = connection.id,
            name = connection.name ?: "未命名连接",
            baseUrl = connection.baseUrl ?: "未知地址",
            username = connection.username ?: "未知用户",
    ...
    fun updateConnection(connection: WebDavConnection) {
        ...
        if (index >= 0) {
            ...
            Log.w("WebDavRepo", "尝试更新一个不存在的连接 ID: ${safeConnection.id}")
            // 或者可以选择添加它？取决于业务逻辑
