    ...
    LazyColumn(modifier = Modifier
        ...
        if (lists.isEmpty()) {
            ...
            items(lists.size) { index ->
                ...
                channelCount = if (audioCode.contains(
                        "ec",
                        true
                    )
                ...
                ListItem(
                    ...
                    headlineContent = {
                        Text(
                            "${
                                Tools.getFullLanguageName(
                                    lists[index].getTrackFormat(
                                        0
                                    ).language
                                )
                            } ${(lists[index].getTrackFormat(0).bitrate) / 1000}Kbps"
                        )
                    },
                    overlineContent = {
                        Text(
                            "${channelCount}声道 · ${
                                Tools.inferAudioFormatType(
                                    lists[index].getTrackFormat(
                                        0
                                    )
                                )
