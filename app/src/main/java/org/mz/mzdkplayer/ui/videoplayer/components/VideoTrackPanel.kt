                ...
                headlineContent = {
                    if (videoCode != "" && videoId != null) {
                        if (!videoCode.contains("dvh", true) && "127" == videoId
                            ...
                            Text("8K 超高清 ${videoHeight}P ${String.format(Locale.CHINA,"%.1f", videoBitmap / 1000.0 / 1000.0)}Mbps")
                        } else if (videoCode.contains("dvh", true) && "126" == videoId) {
                            Text("杜比视界 ${videoHeight}P ${String.format(Locale.CHINA,"%.1f", videoBitmap / 1000.0 / 1000.0)}Mbps")
                        } else if (!videoCode.contains("dvh", true) && "125" == videoId
                        ) {
                            Text("HDR ${videoHeight}P ${String.format(Locale.CHINA,"%.1f", videoBitmap / 1000.0 / 1000.0)}Mbps")
                        }
                        else if (!videoCode.contains("dvh", true) && "120" == videoId
                        ) {
                            Text("4K 超高清 ${videoHeight}P ${String.format(Locale.CHINA,"%.1f", videoBitmap / 1000.0 / 1000.0)}Mbps")
                        } else if (!videoCode.contains("dvh", true) && "116" == videoId
                        ) {
                            Text("1080P60 高帧率 ${videoHeight}P ${String.format(Locale.CHINA,"%.1f", videoBitmap / 1000.0 / 1000.0)}Mbps")
                        } else if (!videoCode.contains("dvh", true) && "112" == videoId
                        ) {
                            Text("1080P+ 高码率 ${videoHeight}P ${String.format(Locale.CHINA,"%.1f", videoBitmap / 1000.0 / 1000.0)}Mbps")
                        }
                        else if (!videoCode.contains("dvh", true) && "100" == videoId
                        ) {
                            Text("智能修复 ${videoHeight}P ${String.format(Locale.CHINA,"%.1f", videoBitmap / 1000.0 / 1000.0)}Mbps")
                        } else if (!videoCode.contains("dvh", true) && "80" == videoId
                        ) {
                            Text("1080P 高清 ${videoHeight}P ${String.format(Locale.CHINA,"%.1f", videoBitmap / 1000.0 / 1000.0)}Mbps")
                        } else if (!videoCode.contains("dvh", true ) && "74" == videoId
                        ) {
                            Text("720P60 高帧率 ${videoHeight}P ${String.format(Locale.CHINA,"%.1f", videoBitmap / 1000.0 / 1000.0)}Mbps" )
                        } else if (!videoCode.contains("dvh", true) && "64" == videoId
                        ) {
                            Text("720P 高清 ${videoHeight}P ${String.format(Locale.CHINA,"%.1f", videoBitmap / 1000.0 / 1000.0)}Mbps")
                        } else if (!videoCode.contains("dvh", true) && "32" == videoId
                                ) {
                            Text("480P 清晰 ${videoHeight}P ${String.format(Locale.CHINA,"%.1f", videoBitmap / 1000.0 / 1000.0)}Mbps" )
                        } else if (!videoCode.contains("dvh", true) && "16" == videoId
                                ) {
                            Text("360P 流畅 ${videoHeight}P ${String.format(Locale.CHINA,"%.1f", videoBitmap / 1000.0 / 1000.0)}Mbps")
                        } else {
                            Text(" ${videoHeight}P ${String.format(Locale.CHINA,"%.1f", videoBitmap / 1000.0 / 1000.0)}Mbps")
                        }
                    }
                },
